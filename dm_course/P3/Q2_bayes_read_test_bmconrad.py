...
# File: Q2_bayes_read_test_bmconrad.py test-shuffled.txt
...
class Bayes_Classifier:
    ...
    def readModel(self):
        ...
        label3 = lines[12]
        cov1_str = lines[13:17] #4 lines
        cov2_str = lines[17:21] #4 lines
        cov3_str = lines[21:] #4 lines
        ...
        cov1_str = [map(float,s1), map(float,s2), map(float,s3), map(float,s4)]       
        ...
        cov2_str = [map(float,s1), map(float,s2), map(float,s3), map(float,s4)]
        ...
        cov3_str = [map(float,s1), map(float,s2), map(float,s3), map(float,s4)]
    ...
    def predict(self, DF_TEST):
        ...
        for j in range(len(self.DF_TEST)):
            ...
            for i in range(self.k):
                #print "Instance j: ", self.DF_TEST.ix[j,:4].as_matrix()
...
# Labels correctly translated into model -- GOOD
# Distribution and algorithm -- NOT SURE -- BUGGY?
# Confusion matrix -- showing me im predicting everything wrong.
def main():
    
    # Import Data
    testName = "data/"
    testName += sys.argv[1] #test-shuffled.txt

    print "Testing Data File Name:", testName
