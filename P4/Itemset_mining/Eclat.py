...
def main():
	...
	if p_flag == 1:
		...
		CUD=[]
		for n in F:
                        _supps.append((n.item_id, n.getSup()))
                        print "support:", str(n.getSup()), "%"
			n.pattern_print();
			for item in n.item_id:
                                if 100 == item:
                                        contains100+=1
                                        AUB.append((n.item_id, n.getSup()))
                                        
                                if 200 == item:
                                        contains200+=1
                                        CUD.append((n.item_id, n.getSup()))
                                        
        print 'Number of frequenct patterns: ', str(len(F))

        _supps.sort(key=lambda x: x[1],reverse=True)
        print 'Top 10 highest support frequent transactions:'
        for i in range(10):
                print _supps[i]
        
        print 'sup(AUB) {head as 100}: ', str(contains100)
        print 'sup(CUD) {head as 200}: ', str(contains200)

        print
        #print AUB
        print 'sup(AUB): ', len(AUB)
        #print CUD
        print 'sup(CUD): ', len(CUD)

        print
        print 'Rules with greater than 75% association (AUB):'
        print AUB[12:17]
        print 'Rules with greater than 75% association (CUD):'
        print CUD[2:8]
        print CUD[16:20]
        A=[]
        for tup in AUB:
                ...
                for n in F:
                        if n.item_id == tup[0][:]:
                                A.append((tup[0][:], tup[1]))

        print 'sup(A): ', str(len(A))
        ...
        for tup in CUD:
                ...
                for n in F:
                        if n.item_id == tup[0][:]:
                                C.append((tup[0][:], tup[1]))

        print 'sup(C): ', str(len(C))

        #conf1 =  len(AUB)/len(A)
        #conf2 = len(CUD)/len(C)
        #print 'conf(AUB) = ', conf1
        #print 'conf(CUD) = ', conf2
        
        topass = [tup[0] for tup in AUB[:10]]
        topass2 = [tup[0] for tup in CUD[:10]]
        print 'Top 10 association rules with 100 as the head', topass
        print 'Top 10 association rules with 200 as the head', topass2
